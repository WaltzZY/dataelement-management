...
/**
 * Created by liujf01 on 2018/6/6.
 */
...
public class CacheUtils {
     ...
     * @return            缓存的对象
     */
    public  static <T> BoundListOperations<String, T> setCacheList(String key, List<T> dataList, ExpireTime time) {
        BoundListOperations boundListOperation = redisTemplate.boundListOps(key);
        if(null != dataList)
        {
            int size = dataList.size();
            for(int i = 0; i < size ; i ++) {
                boundListOperation.rightPush(dataList.get(i));
            }
        }
        if(time.getTime() > 0){
            redisTemplate.expire(key, time.getTime(), TimeUnit.SECONDS);
        }
        return boundListOperation;
    }
    ...
    public static List<Object> getCacheList(String key) {
        ...
        BoundListOperations<String, Object> boundListOperation = redisTemplate.boundListOps(key);
        dataList = boundListOperation.range(0, -1);
        return dataList;
    }

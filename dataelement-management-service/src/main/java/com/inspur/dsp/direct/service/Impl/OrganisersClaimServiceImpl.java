...
import com.inspur.dsp.direct.dao.OrganisersClaim.ClaimConfirmationTaskMapper;
...
public class OrganisersClaimServiceImpl implements OrganisersClaimService {
    ...
    private final ClaimConfirmationTaskMapper claimConfirmationTaskMapper;
    ...
    public Page<ClaimDataElementVO> getDataElement(GetDataElementDTO dto) {
        ...
        Page<ClaimDataElementVO> list = claimDataElementMapper.selectBaseDataElementByStatus(page, dto);

        // 循环遍历List<ClaimDataElementVO>
        for (ClaimDataElementVO vo : list.getRecords()) {
...
//                Date now = Date.from(Instant.now());
                ...
                for (DomainDataElement dde : domainDataElements) {
                    ...
                    confirmationTask.setStatus(CLAIMED.getCode());
                    ...
                    claimConfirmationTaskMapper.insertById(confirmationTask);
                }
    ...
    public void exportData(GetDataElementDTO dto, HttpServletResponse response) {
        ...
        try {
            if (dto.getStatus() != null && dto.getStatus().contains("pending_source")) {
                ...
                // 查询待定源数据元列表
                Page<ClaimDataElementVO> list_pendingSource = claimDataElementMapper.selectBaseDataElementByStatus(null, dto);

                // 循环遍历List<ClaimDataElementVO>
                for (ClaimDataElementVO vo : list_pendingSource.getRecords()) {
                    // 使用ClaimDataElementVO.dataid作为参数，获取关联的DomainDataElement列表
                    List<DomainDataElement> domainDataElements =
                            claimDomainDataElementMapper.selectDomainDataElementByBaseDataElementDataId(vo.getDataid());
                ...
                List<OrganisersClaimPendingExcel> pendingExcelList = list_pendingSource.getRecords().stream().map(vo -> {
                    return OrganisersClaimPendingExcel.builder()
                            .name(vo.getName())
                            .definition(vo.getDefinition())
                            .collectQty(String.valueOf(vo.getCollectunitqty()))
                            .collectUnit(vo.getCollectunitnames())
                            .status(vo.getStatus())
                            .build();
            ...
            } else if (dto.getStatus() != null && dto.getStatus().contains("confirming")) {
                ...
                List<OrganisersClaimingExcel> pendingExcelList = claimDataElementVOS.stream().map(vo -> {
                    return OrganisersClaimingExcel.builder()
                            .name(vo.getName())
                            .definition(vo.getDefinition())
                            .collectQty(String.valueOf(vo.getCollectunitqty()))
                            .collectUnit(vo.getCollectunitnames())
